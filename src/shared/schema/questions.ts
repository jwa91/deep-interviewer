...
export const RecordAiBackgroundSchema = z
	.object({
		userType: UserTypeSchema.describe(
			"Geschatte gebruikerstype op basis van gesprek",
		),
...
export type RecordOverallImpression = z.infer<
	typeof RecordOverallImpressionSchema
>;
...
export const RecordPerceivedContentSchema = z
	.object({
		mainTopicsIdentified: z
			.array(z.string())
			.describe("Hoofdonderwerpen die deelnemer noemt"),
		alignsWithIntent: z
			.boolean()
			.describe("Komt overeen met bedoelde leerdoelen?"),
	})
	.merge(QualitativeCaptureSchema);

export type RecordPerceivedContent = z.infer<
	typeof RecordPerceivedContentSchema
>;
...
export const RecordDifficultySchema = z
	.object({
		...
		difficultyLevel: DifficultyLevelSchema.describe(
			"Categorisatie moeilijkheidsgraad",
		),
		paceRating: RatingSchema.describe(
			"1=veel te langzaam, 3=precies goed, 5=veel te snel",
		),
	})
	.merge(QualitativeCaptureSchema);
...
export const RecordContentQualitySchema = z
	.object({
		...
		relevanceRating: RatingSchema.describe(
			"1=niet relevant, 5=zeer relevant voor mijn werk",
		),
		depthRating: RatingSchema.describe(
			"1=te oppervlakkig, 3=precies goed, 5=te diepgaand",
		),
	})
	.merge(QualitativeCaptureSchema);
...
export const RecordClaritySchema = z
	.object({
		...
		needsExplanation: z
			.boolean()
			.describe("Wil deelnemer dat iets opnieuw wordt uitgelegd?"),
		explanationTopic: z
			.string()
			.optional()
			.describe("Welk onderwerp moet uitgelegd worden?"),
	})
	.merge(QualitativeCaptureSchema);
...
export const RecordSuggestionsSchema = z
	.object({
		...
		topSuggestion: z
			.string()
			.optional()
			.describe("Belangrijkste suggestie volgens deelnemer"),
		wouldRecommend: z
			.boolean()
			.describe("Zou deelnemer de training aanraden?"),
	})
	.merge(QualitativeCaptureSchema);
...
export const RecordCoursePartsSchema = z
	.object({
		preferredPart: CoursePartPreferenceSchema.describe(
			"Welk deel vond deelnemer leuker/nuttiger?",
		),
		theoryPartRating: RatingSchema.describe(
			"1=niet nuttig, 5=zeer nuttig - het theoretische deel",
		),
		...
		balanceOpinion: z
			.string()
			.optional()
			.describe("Mening over de balans tussen beide delen"),
	})
	.merge(QualitativeCaptureSchema);

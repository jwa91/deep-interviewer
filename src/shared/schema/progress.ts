...
export function createEmptyQuestionCompletion(): QuestionCompletion {
  return Object.fromEntries(QUESTION_IDS.map((id) => [id, false])) as QuestionCompletion;
}
 ...
 */
export function getRemainingQuestions(completion: QuestionCompletion): QuestionIdType[] {
 ...
 */
export function getCompletedQuestions(completion: QuestionCompletion): QuestionIdType[] {
  return QUESTION_IDS.filter(
    (id) => completion[id as keyof QuestionCompletion]
  ) as QuestionIdType[];
}

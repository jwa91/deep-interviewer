...
import { QUESTION_IDS } from "../../shared/schema/index.js";
...
interviewRoutes.post("/:id/chat", async (c) => {
  ...
  // Stream the response
  // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: SSE stream handler is intentionally stateful.
  return streamSSE(c, async (stream) => {
    try {
      // Use streaming mode
      ...
      for await (const event of eventStream) {
        ...
        if (event.event === "on_chat_model_stream") {
          ...
          // If this happens, the model streamed user-facing text before tool execution.
          // The system prompt is designed to prevent this; keep this warning so we can
          // detect regressions without sacrificing streaming UX.
          if (currentMessageContent.trim()) {
            console.warn("Model streamed text before tool_start; prompt may be violated.");
          }
...
interviewRoutes.get("/:id/responses/:topic", async (c) => {
  ...
  // Validate topic is a valid question ID (using QUESTION_IDS as single source of truth)
  if (!QUESTION_IDS.includes(topic as (typeof QUESTION_IDS)[number])) {

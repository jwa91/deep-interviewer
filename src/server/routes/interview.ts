	...
	try {
		...
		const messages = (state.values?.messages ?? [])
			...
			.filter((msg: { content: string; toolCalls?: unknown[] }) => {
				...
				return (
					msg.content.length > 0 || (msg.toolCalls && msg.toolCalls.length > 0)
				);
			});

		const questionsCompleted = state.values?.questionsCompleted ?? {};
		const completedCount =
			Object.values(questionsCompleted).filter(Boolean).length;
...
interviewRoutes.post("/:id/chat", async (c) => {
	...
	let input:
		...
		| ReturnType<typeof createInterviewInput>;
		...
		for (const [topic, data] of Object.entries(responses)) {
			if (data) {
				responsesWithMetadata[topic] = {
					...
					timestamp:
						state.values?.completedAt ??
						state.values?.startedAt ??
						new Date().toISOString(),
					// Check if data has a source field, otherwise default to "agent"
					source:
						(data as { source?: "agent" | "user_edit" }).source ?? "agent",
				};
			}
		}

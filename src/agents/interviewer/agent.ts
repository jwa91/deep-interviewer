...
export function createInterviewAgent(config: InterviewAgentConfig = {}) {
	...
	async function callModel(
		...
		const remainingIds = getRemainingQuestionIds({
			...state,
			questionsCompleted:
				questionsCompleted as InterviewState["questionsCompleted"],
		});
	...
	// biome-ignore lint/suspicious/noExplicitAny: LangGraph StateGraph requires string literal casts for node names
	graph.addConditionalEdges("model" as any, shouldContinue, [
		// biome-ignore lint/suspicious/noExplicitAny: LangGraph StateGraph requires string literal casts
		"tools" as any,
		END,
	]);
	// biome-ignore lint/suspicious/noExplicitAny: LangGraph StateGraph requires string literal casts for node names
